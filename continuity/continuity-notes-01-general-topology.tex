...
I think it makes sense pedagogically to start with a chapter introducing topology of finite sets of discrete points. Probably graphs are the easiest to visualize. Model a graph as a system of open sets and show how we can use this to characterise some basic connectedness properties and describe a continuous map between graphs. 

Keep this short and emphasise that we want to translate these concepts to continua, which aren't made of points the way graphs are -- that makes the task a lot more tricky.

\section{Two Views of Graphs}

Show a drawing of a graph and its `natural' representation as sets of vertices and edges.

Then show an alternative model in terms of open sets. 

Show several examples we can re-use, e.g. a tree, a cyclic graph, a graph with two components.

\section{Connectedness}

Show how to determine whether a graph is connected and how to count components.

\section{Continuous Maps and Isomorphisms}

Define a continuous map.

Say when a continuous map represents an isomorphism

Show an example of non-isomorphic graphs, and one of isomorphic graphs that isn't visually obvious.
...
The specific view of topological spaces as continua is elaborated in more detail in \textbf{Topology, Logic and Stone Duality} and a special version of it underpins the courses in the Smooth Manifolds block. The traditional point-set view, on the other hand, gives rise to a strictly larger theory whose outer reaches are explored in \textbf{Topics in General Topology}.
...
\begin{defn}
	An \textbf{$n$-fold even cover} of an open set $U$ in a topological space is a cover of $U$ such that every point in $U$ has a neighbourhood that is a subset of $n$ elements of the cover, where $n$ has the same value at every point.
\end{defn}
