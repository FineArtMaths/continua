...
\begin{defn} 
	A \textbf{system of open sets} $S$ is a collection of open sets obeying the following axioms:
	\begin{itemize}
...
\begin{defn} 
	A \textbf{topological space} is some object we would like to consider as a continuum; to achieve this we equip it with a system of open sets. We interpret $\top$ as representing the whole space and $\bot$ the `empty set'; intermediate open sets represent distinguishable or observable parts of the continuum.
\end{defn} 
...
Note that the filters might be infinite, so that the topological point it produces is not necessarily an open set.

In general topology, the definition of a point is different. The space $X$, in addition to its system of open sets, is assumed to contain a set of points, which I'll call its `general points'. Then problems arise then a topology does not have enough open sets to give rise to all those points as topological points. In almost all the topological spaces that interest us here and in future, there are `enough open sets' and the notion of topological point and general point coincide. We study spaces where this does not happen in \textbf{Topics in General Topology}.
...
\begin{defn} 
	A \textbf{closed set} $C$ of a topological space $X$ is a set of topological points such that $X\setminus C$ is an open set.
\end{defn} 
...
If topological spaces are to form a category -- which is the kind of thing we 
