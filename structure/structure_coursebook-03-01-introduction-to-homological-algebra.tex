...
\begin{document}
	...
	\chapter{Introduction to Modules}
			...
			\section{Left vs Right}
				...
				It turns out that the dual of a right $R$-module is a left $R$-module and \emph{vice versa}. When $R$ is non-commutative, the dual of each of its modules is not of `the same kind' as the original. This creates wrinkles in the theory that we don't have to worry about when $R$ is commutative, as in the case of vector spaces. We'll see some consequences of this in the first half of this course but make a more systematic study of the problem in the second half, when we recast the idea of duality in terms of `opposite categories'.
	...
	\chapter{Hom and Tensor}
		...
		\section{Projective and Injective Modules}
			...
			\subsection{Projective Modules}
				...
				Example: $\mathbb{Z}/2\mathbb{Z}$ as a module over $\mathbb{Z}/6\mathbb{Z}$. It is projective but couldn't be a free module as it only has two elements, so it can't a a direct sum of copies of the six-element $\mathbb{Z}/6\mathbb{Z}$.
		...
		\section{Change of Rings}
			...
			\subsection{Co-Extension of Scalars (*)}
				...
				This section is included for neatness and symmetry although we will never need this construction in practice. The idea is that just as extension of scalars is left-adjoint to restriction, there is a dual right-adjoint known as co-extension. It will probably not astonish you to learn that in place of the tensor product, co-extension uses Hom.
	...
	\chapter{Extensions}
		...
		\section{Extensions}
			\subsection{Extensions of Modules}
				...
				Specifically call out the danger of confusion with extension of modules!
